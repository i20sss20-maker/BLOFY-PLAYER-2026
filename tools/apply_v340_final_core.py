...
# -----------------------------------------------------------------------------
# 1) Provider-scoped compatibility preflight.
# It runs only after the full catalog has been committed locally. It samples more
# than one item per content family and resolves canonical/direct/no-extension.
# This does not stream media bytes through BLOFY; it validates the exact native-link
# path the player will use and stores the learned compatibility baseline locally.
# -----------------------------------------------------------------------------
...
final class ServerCompatibilityPreflight {
    ...
    static final class Result {
        ...
        Result(int liveScore, int moviesScore, int seriesScore,
               int tested, int playable, String summary, String report) {
    ...
    private static int[] testFamily(Context context, BlofyApi api, CatalogDatabase database,
        ...
        for (BlofyModels.Media item : items) {
            tested++;
            boolean ok = testItem(context, api, type, item, report);
            if (ok) playable++;
        }
    ...
    private static boolean testItem(Context context, BlofyApi api, String type,
        ...
        String baseExt = PlaybackPolicy.normalizeExtension(item.extension,
            ...
            for (String variant : variants) {
                ...
                try {
                    JSONObject data = api.getPlayback("/api/native-link/" + BlofyApi.encode(apiType) + "/"
                            ...
                            + "&variant=" + BlofyApi.encode(variant), cancellation);
                    ...
                    if (url.startsWith("http://") || url.startsWith("https://")) {
                        ...
                        report.append(type).append('/').append(item.id)
                                .append(" OK route=").append(variant)
                                .append(" ext=").append(resolvedExt)
                                .append(" resolve_ms=").append(elapsed).append('\n');
                        PlaybackDiagnostics.marker(context, "preflight-route-ok", type, item.id,
                                resolvedExt, variant, "resolve_ms=" + elapsed);
                        return true;
                    }
                    last = "invalid-url";
        ...
        long elapsed = Math.max(0L, SystemClock.elapsedRealtime() - started);
        report.append(type).append('/').append(item.id)
                .append(" FAIL ").append(last).append(" elapsed_ms=").append(elapsed).append('\n');
        PlaybackDiagnostics.marker(context, "preflight-route-fail", type, item.id,
                baseExt, "all-routes", last + " elapsed_ms=" + elapsed);
        return false;
    }
    ...
    private static String key(String playlistId) {
        String raw = safe(playlistId).trim().toLowerCase(Locale.US);
...
# -----------------------------------------------------------------------------
# 2) Full initial sync stays authoritative. Only after every catalog family is
# committed do we run preflight. Cache reuse remains instant on later connections.
# -----------------------------------------------------------------------------
...
# Expose app context internally to the preflight helper without forwarding credentials.
api_path = JAVA / "BlofyApi.java"
...
# -----------------------------------------------------------------------------
# 3) Bounded startup windows: dead routes fail fast, but UHD/VOD still get a
# realistic window. The existing recovery ladder remains unchanged.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5) Settings shows the last compatibility baseline separately from raw diagnostics.
# -----------------------------------------------------------------------------
settings = SETTINGS.read_text(encoding="utf-8")
anchor = '''        addGridSetting(grid, gridAction("▤  تقرير التشخيص", "عرض كامل • تصوير أو نسخ",\n                this::showDiagnosticsReport));\n'''
addition = anchor + '''        addGridSetting(grid, gridAction("◎  توافق السيرفر",\n                ServerCompatibilityPreflight.savedSummary(this, new PlaylistStore(this).activeId()), () -> {\n            String report = ServerCompatibilityPreflight.savedReport(\n                    this, new PlaylistStore(this).activeId());\n            android.content.ClipboardManager clipboard =\n                    (android.content.ClipboardManager) getSystemService(CLIPBOARD_SERVICE);\n            if (clipboard != null) clipboard.setPrimaryClip(\n                    android.content.ClipData.newPlainText("BLOFY compatibility", report));\n            ToastBridge.show(this, "تم نسخ تقرير توافق السيرفر");\n        }));\n'''
settings = replace_once(settings, anchor, addition, 'compatibility settings action')
...
for path, tokens in [
    ...
    for token in tokens:
        if token not in text:
            raise SystemExit(f"v340 final core invariant missing in {path.name}: {token}")
