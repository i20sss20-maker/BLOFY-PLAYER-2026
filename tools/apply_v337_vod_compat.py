...
# Learned transport starts a previously proven item on Cronet directly, without
# changing the source URL/extension. A failed learned route can still recover.
patch(
...
# Cronet existed but VOD always forced platform HTTP. Only compatibility/learned
# step 1 prefers Cronet; unavailable Cronet safely falls back in the factory.
patch(
...
'''        boolean preferCronet = attempt == 1 && PlaybackTransportFactory.isCronetReady();\n        DataSource.Factory source = PlaybackTransportFactory.create(\n                this, preferCronet, network,\n                attempt == 0 ? (ultraHd() ? 12_000 : 8_000) : 4_000,\n                attempt == 0 ? (ultraHd() ? 28_000 : 20_000) : 12_000,\n                attempt, playbackReferer);\n''', "activate bounded VOD Cronet")

# Expose actual selected network engine in the small player diagnostics chip.
patch(
...
# Resolve errors for optional direct/no-extension routes should not become a
# fatal modal until canonical + compatibility player have also been exhausted.
patch(
...
# LibVLC Time/Position is our successful-frame proxy for VOD. Persist that a
# compatibility player was needed at provider level, while per-item network
# transport remains unchanged (LibVLC is the final player fallback, not HTTP).
patch(
'''                if (!firstFrame) {\n                    firstFrame = true;\n                    spinner.setVisibility(View.GONE);\n                    main.removeCallbacks(startupTimeout);\n                    showControls();\n                }\n''',
