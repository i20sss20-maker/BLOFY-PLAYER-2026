...
final class PlaybackPreloadManager {
    ...
    private static final LinkedHashMap<String, PreloadedRoute> CACHE = new LinkedHashMap<String, PreloadedRoute>(8, 0.75f, true) {
        @Override protected boolean removeEldestEntry(Map.Entry<String, PreloadedRoute> eldest) { return size() > MAX; }
    };
    private static final class PreloadedRoute {
        final JSONObject value; final long at;
        PreloadedRoute(JSONObject value) { this.value = value; this.at = SystemClock.elapsedRealtime(); }
    }
    private PlaybackPreloadManager() {}
    static void preload(Context context, String path) {
        if (path == null || path.isEmpty()) return;
        synchronized (CACHE) {
            PreloadedRoute existing = CACHE.get(path);
        ...
        WORKER.execute(() -> {
            try {
                ...
                synchronized (CACHE) { CACHE.put(path, new PreloadedRoute(new JSONObject(value.toString()))); }
    ...
    static JSONObject take(String path) {
        synchronized (CACHE) {
            PreloadedRoute entry = CACHE.remove(path);
...
checks = {
    PRELOAD: ["r11d-preload-ready", "TTL_MS = 20_000L", "MAX = 6", "PreloadedRoute"],
