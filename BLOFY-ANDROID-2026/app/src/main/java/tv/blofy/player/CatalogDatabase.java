...
final class CatalogDatabase extends SQLiteOpenHelper {
    ...
    List<BlofyModels.Media> featured(int limit) {
        ...
        String sql = "SELECT " + MEDIA_COLUMNS + " FROM media m " +
                "WHERE m.type IN ('movies','series') AND (m.backdrop<>'' OR m.image<>'') " +
                "ORDER BY CASE WHEN m.rating<>'' THEN 0 ELSE 1 END," +
                ...
                "m.release_date DESC,CAST(m.year AS INTEGER) DESC,m.sort_order DESC LIMIT ?";
