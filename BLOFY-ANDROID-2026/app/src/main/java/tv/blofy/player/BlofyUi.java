...
/**
 * BLOFY TV design language.
 *
 * Uses the same television-first visual principles observed in 7 Max — dark
 * canvas, restrained panels, large remote targets and an obvious focus ring —
 * while keeping BLOFY's own purple identity and assets.
 */
final class BlofyUi {
    ...
    static final int MUTED = Color.rgb(184, 181, 192);
    ...
    static TextView text(Context context, String value, int sp, int color) {
        ...
        view.setPadding(dp(context, 9), dp(context, 5), dp(context, 9), dp(context, 5));
        return view;
    }
    ...
    static EditText input(Context context, String hint, boolean numeric) {
        ...
        view.setHintTextColor(Color.rgb(137, 134, 145));
        view.setTextSize(15);
        view.setPadding(dp(context, 18), 0, dp(context, 18), 0);
        view.setBackground(focusDrawable(context, Color.rgb(42, 41, 47), Color.rgb(57, 49, 72), Color.WHITE));
    ...
    static Button button(Context context, String label, boolean primary) {
        ...
        view.setPadding(dp(context, 17), 0, dp(context, 17), 0);
        view.setBackground(primary
                ? focusDrawable(context, PURPLE, Color.rgb(139, 77, 247), Color.WHITE)
                : focusDrawable(context, PANEL_ALT, Color.rgb(55, 48, 68), Color.WHITE));
        view.setFocusable(true);
        return view;
    }
    ...
    static Drawable panel(Context context, int color, int radiusDp, int strokeColor) {
        ...
        drawable.setCornerRadius(dp(context, Math.min(radiusDp, 12)));
        drawable.setStroke(dp(context, 1), Color.argb(205, Color.red(strokeColor), Color.green(strokeColor), Color.blue(strokeColor)));
        return drawable;
    }
    ...
    static Drawable focusDrawable(Context context, int normal, int focused, int focusStroke) {
        ...
        focus.setCornerRadius(dp(context, 11));
        focus.setStroke(dp(context, 3), focusStroke);
        GradientDrawable idle = new GradientDrawable();
        idle.setColor(normal);
        idle.setCornerRadius(dp(context, 11));
        idle.setStroke(dp(context, 1), Color.rgb(67, 65, 73));
        ...
        return new Drawable() {
            ...
            @Override public void draw(android.graphics.Canvas canvas) {
                paint.setShader(new LinearGradient(0, 0, canvas.getWidth(), canvas.getHeight(),
                        new int[]{Color.rgb(5, 5, 8), Color.rgb(14, 10, 20), Color.rgb(6, 5, 10)},
    ...
    static LinearLayout brand(Context context, String subtitle) {
        ...
        row.addView(logo, new LinearLayout.LayoutParams(dp(context, 68), dp(context, 68)));

        LinearLayout labels = new LinearLayout(context);
        labels.setOrientation(LinearLayout.VERTICAL);
        TextView name = title(context, "BLOFY", 22);
        name.setTextDirection(View.TEXT_DIRECTION_LTR);
        name.setGravity(Gravity.LEFT);
        TextView player = text(context, subtitle == null ? "P L A Y E R" : subtitle, 10, PURPLE_LIGHT);
