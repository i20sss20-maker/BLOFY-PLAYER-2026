...
/**
 * Bounded compatibility discovery for one playlist.
 * It never downloads the full catalogue or media payload: probes are short and response bodies are capped.
 */
public final class ServerDiscoveryEngine {
    ...
    public CompatibilityProfile discover(PlaylistProfile playlist, Listener listener) throws Exception {
        ...
        ProviderProfile provider = playlist.provider;
        progress(listener, 3);

        if (provider.isM3u()) {
            return discoverM3u(playlist, provider.playlistUrl, listener);
        }
        return discoverXtream(playlist, provider, listener);
    }
    ...
    private CompatibilityProfile discoverXtream(PlaylistProfile playlist, ProviderProfile provider,
        ...
        Probe auth = probe(api, "application/json,text/plain,*/*", false);
        progress(listener, 26);

        if (auth.ok && looksLikeXtream(auth.preview)) {
            HttpUrl categories = api.newBuilder().addQueryParameter("action", "get_live_categories").build();
            Probe live = probe(categories, "application/json,text/plain,*/*", false);
            progress(listener, 43);

            HttpUrl vodCategories = api.newBuilder().setQueryParameter("action", "get_vod_categories").build();
            Probe vod = probe(vodCategories, "application/json,text/plain,*/*", false);
            progress(listener, 58);

            HttpUrl seriesCategories = api.newBuilder().setQueryParameter("action", "get_series_categories").build();
            Probe series = probe(seriesCategories, "application/json,text/plain,*/*", false);
        ...
        // API blocked or incompatible: probe standard Xtream M3U endpoint using the same credentials.
        progress(listener, 36);
        HttpUrl get = HttpUrl.parse(provider.serverUrl + "/get.php");
        if (get == null) throw new Exception("Invalid Xtream fallback URL");
        get = get.newBuilder()
                .addQueryParameter("username", provider.username)
                .addQueryParameter("password", provider.password)
    ...
    private CompatibilityProfile discoverM3u(PlaylistProfile playlist, String url, Listener listener) throws Exception {
        ...
        Probe m3u = probe(target, "application/x-mpegURL,text/plain,*/*", true);
        progress(listener, 58);
        if (!m3u.ok || !looksLikeM3u(m3u.preview)) {
    ...
    private static boolean looksLikeM3u(String value) {
        return value != null && (value.startsWith("#EXTM3U") || value.contains("#EXTINF"));
    }
    ...
    public static final class DiscoveryException extends Exception {
        ...
        DiscoveryException(int httpStatus, String message) {
            super(message);
